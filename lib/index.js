...
module.exports = {
  ...
  treeForPublic() {
    ...
    if (this.hasHbsStrategy() && this.optionFor('hbs', 'bundle') === false) {
  ...
  treeForAddon(addonTree) {
    ...
    if (this.hasHbsStrategy()) {
      const hbsTree = this.getHbsStrategyTree();
      if (this.optionFor('hbs', 'bundle')) {
        trees.push(hbsTree);
      } else {
        this.options.createdComponents.push(
          this._super.treeForAddon.call(this, hbsTree)
        );
      }
    }
